...
*/
  const setDeviceIdToStorage = async (deviceid) => {
    try {
      ...
      const id = await AsyncStorage.setItem('@DEVICE_ID', JSON.stringify(deviceid))
    } catch (e) {
      console.log("Error on getting Token from Asyn storage (Home Settings screen): ", e);
    }
  }
  ...
  */
  ...
  useEffect(() => {
    getHomeToStorage().then((home) => {
      let homeData = JSON.parse(home)
      setHomeId(homeData.id)
      setSelecedItem(homeData)
    }).catch(e => console.log("Error on setting home data from local storage: ", e))
    getDeviceIdFromStorage().then((device) => {
      let deviceData = JSON.parse(device);
      setDevice(deviceData)
      setDeviceId(deviceData.id)
    }).catch(e => console.log("Error on setting device data from local storage: ", e))
  }, [])
