...
import { View, Text, StyleSheet, SafeAreaView, TextInput, ActivityIndicator, Dimensions, Alert } from 'react-native'
  ...
  const postHome = () => {
    if (homeName != "") {
      getToken().then((token) => {
        ...
        Alert.alert(
          'Error',
          `Error on creating home, try again.`,
          [
            { text: 'OK' },
          ]
        );
      ...
      Alert.alert(
        'Empty',
        `Please enter home name and try again.`,
        [
          { text: 'OK' },
        ]
      );
    ...
    <SafeAreaView style={style.container} >
      ...
      <View style={style.textAreaWrapper} >
        <TextInput
          ...
          onChangeText={(homeName) => { setHomeName(homeName) }}
          value={homeName}
        ></TextInput>
      </View>
